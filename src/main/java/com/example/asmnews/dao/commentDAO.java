...
public class commentDAO {
    ...
    public List<Comment> findByNewsId(String newsId) {
        ...
        String sql = "SELECT c.Id, c.Content, c.CreatedDate, c.UserId, c.NewsId, u.Fullname AS UserFullName " +
                "FROM Comments c " +
        ...
        try (Connection conn = DatabaseUtils.getConnection();
            ...
            System.err.println("Lỗi load Comments (findByNewsId): " + e.getMessage());
            e.printStackTrace();
        }
        return list;
    }
    ...
    public boolean update(int commentId, String userId, String newContent) {
        String sql = "UPDATE Comments SET Content = ? WHERE Id = ? AND UserId = ?";
        try (Connection conn = DatabaseUtils.getConnection();
                PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public boolean reportComment(int commentId) {
        String sql = "UPDATE Comments SET ReportCount = ISNULL(ReportCount, 0) + 1 WHERE Id = ?";
        try (Connection conn = DatabaseUtils.getConnection();
                PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public List<Comment> findAllForAdmin() {
        List<Comment> list = new ArrayList<>();
        String sql = "SELECT c.Id, c.Content, c.CreatedDate, c.UserId, c.NewsId, ISNULL(c.ReportCount, 0) as ReportCount, "
                +
                "u.Fullname AS UserFullName, n.Title AS NewsTitle " +
                "FROM Comments c " +
                "INNER JOIN Users u ON c.UserId = u.Id " +
                "INNER JOIN News n ON c.NewsId = n.Id " +
